...
export function designHintForType(type: ProductionAssetType): string {
  ...
  if (type === "scene_design") {
    return "场景设计会按环境设定表生成：主视角、反打/侧向、入口动线、关键道具、材质和灯光细节，用来锁定后续镜头的一致性。";
  }
...
export function examplePromptForType(type: ProductionAssetType): string {
  ...
  if (type === "scene_design") {
    return "生成一个雨夜便利店的场景设计设定表。冷白荧光灯，玻璃门外有雨水反光，货架排列清楚，收银台、旧监控屏、咖啡机作为固定道具。需要同一地点的主建立镜头、侧向视角、入口动线、收银台细节、咖啡机和监控屏特写，所有角度保持同一空间布局、同一灯光和同一道具位置。无文字、无招牌文字、无故事分镜。";
  }
