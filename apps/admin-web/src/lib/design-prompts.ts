...
export function buildDesignPromptForType(type: ProductionAssetType, prompt: string): string {
  ...
  if (type === "scene_design") {
    return [
      ...
      "输出规格：专业场景设计设定表 / environment design sheet，一张图内包含同一地点的多个一致视角和细节。",
      "必须包含：主建立镜头、反打或侧向视角、入口/动线视角、无文字空间平面关系、关键道具特写、材质与灯光细节。所有视角必须属于同一空间。",
      "一致性要求：门窗、家具、道具、主要色彩、灯光方向、空间比例、时代风格必须在所有视角中保持一致。",
      "如果展示空间关系，使用无文字的俯视布局或平面构图表达动线，不要生成标签文字或说明框。",
...
export function designHintForType(type: ProductionAssetType): string {
  ...
  if (type === "scene_design") {
    return "场景设计会按多角度环境设定表生成：主视角、反打/侧向、入口动线、无文字空间关系、关键道具、材质和灯光细节，用来锁定后续镜头的一致性。";
  }
...
export function examplePromptForType(type: ProductionAssetType): string {
  ...
  if (type === "scene_design") {
    return "生成一个雨夜便利店的多角度场景设计设定表。冷白荧光灯，玻璃门外有雨水反光，货架排列清楚，收银台、旧监控屏、咖啡机作为固定道具。需要同一地点的主建立镜头、侧向视角、入口动线、无文字俯视空间关系、收银台细节、咖啡机和监控屏特写，所有角度保持同一空间布局、同一灯光和同一道具位置。无文字、无招牌文字、无故事分镜。";
  }
