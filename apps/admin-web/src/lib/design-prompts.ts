...
export function buildDesignPromptForType(type: ProductionAssetType, prompt: string): string {
  ...
  if (type === "scene_design") {
    return [
      ...
      "输出规格：专业场景设计设定表 / environment bible sheet，一张图内包含同一地点的多个一致视角和细节，不是单张背景图。",
      "必须包含：主建立镜头、反打或侧向视角、低机位或高机位辅助视角、入口/动线视角、无文字俯视空间关系、关键道具特写、材质与灯光细节、可复用机位范围。所有视角必须属于同一空间。",
...
export function designHintForType(type: ProductionAssetType): string {
  ...
  if (type === "scene_design") {
    return "场景设计会按 Environment Bible 生成：主视角、反打/侧向、辅助机位、入口动线、无文字俯视空间关系、关键道具、材质和灯光细节，用来锁定后续镜头的一致性。";
  }
...
export function examplePromptForType(type: ProductionAssetType): string {
  ...
  if (type === "scene_design") {
    return "生成一个雨夜便利店的 Environment Bible 场景设定表。冷白荧光灯，玻璃门外有雨水反光，货架排列清楚，收银台、旧监控屏、咖啡机作为固定道具。需要同一地点的主建立镜头、反打/侧向视角、低机位或高机位辅助视角、入口动线、无文字俯视空间关系、收银台细节、咖啡机和监控屏特写、材质灯光细节与可复用机位范围。所有角度保持同一空间布局、同一灯光和同一道具位置。无文字、无招牌文字、无故事分镜。";
  }
